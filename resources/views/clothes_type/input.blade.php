<div class="form-group">
    ...
    <div class="col-sm-9 col-lg-10 controls">
      <select class="form-control chosen-rtl" name="service_id[]" multiple required>
        @foreach($services as $service)
            <option value="{{$service->id}}" {{$clothes_type && in_array($service->id, $clothes_type->services()->pluck('service_id')->toArray()) ? 'selected' : '' }}>{{$service->getTranslation('name', Session::get('applocale'))}}</option>
        @endforeach
      </select>
    </div>
</div>
        ...
        <div class="tab-content">
            ...
            @foreach ($languages as $language)
                <div class="tab-pane fade in {{ $i++ ? '' : 'active' }}" id="name{{ $language->short_code }}">
                    <input class="form-control" name="name[{{ $language->short_code }}]" value="@if ($clothes_type) {!! $clothes_type->getTranslation('name', $language->short_code) !!} @endif" />
                </div>
            @endforeach
        </div>
    </div>
            ...
            @foreach ($languages as $language)
                <div class="tab-pane fade in {{ $i++ ? '' : 'active' }}"
                    ...
                    <textarea class="form-control col-md-12"
                        ...
                        rows="6">{{ $clothes_type!=null ? $clothes_type->getTranslation('description', $language->short_code) : old('description.' . $language->short_code)}}</textarea>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="col-sm-9 col-md-8 controls">
        <div class="fileupload fileupload-new" data-provides="fileupload">
            <div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
                @if($clothes_type)
                    <img src="{{url($clothes_type->image)}}" alt="" />
                @else
