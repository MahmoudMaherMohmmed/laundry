        ...
        <div class="tab-content">
            ...
            @foreach ($languages as $language)
                <div class="tab-pane fade in {{ $i++ ? '' : 'active' }}" id="name{{ $language->short_code }}">
                    <input class="form-control" name="name[{{ $language->short_code }}]" value="@if ($service) {!! $service->getTranslation('name', $language->short_code) !!} @endif" />
                </div>
            @endforeach
        </div>
    </div>
            ...
            @foreach ($languages as $language)
                <div class="tab-pane fade in {{ $i++ ? '' : 'active' }}"
                    ...
                    <textarea class="form-control col-md-12"
                        ...
                        rows="6">{{ $service!=null ? $service->getTranslation('description', $language->short_code) : old('description.' . $language->short_code)}}</textarea>
                </div>
            @endforeach
        </div>
    </div>
